...
RegisterNetEvent('msfw-banking:createNewCard', function()
    local src = source
...
--[[ -- Only used by the following "msfw-banking:initiateTransfer"
...
RegisterNetEvent('msfw-banking:initiateTransfer', function(_)
    --[[
    local _src = source
    ...
    if (checkAccount) then
        ...
        if receiptName ~= false or receiptName ~= nil then
            ...
            if userOnline ~= false then
                ...
                if acType == "Current" then
                    ...
                    TriggerClientEvent('msfw-banking:openBankScreen', _src)
                    TriggerClientEvent('msfw-banking:successAlert', _src, 'You have sent a bank transfer to '..receiptName..' for the amount of $'..data.amount)
                else
        ...
        TriggerClientEvent('msfw-banking:transferError', _src, 'The account details entered could not be located.')
    end
]]
end)
...
QBCore.Functions.CreateCallback('msfw-banking:getBankingInformation', function(source, cb)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('msfw-banking:createBankCard', function(pin)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    local cid = xPlayer.PlayerData.citizenid
    ...
    TriggerClientEvent('msfw-banking:openBankScreen', src)
...
RegisterNetEvent('msfw-banking:doQuickDeposit', function(amount)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentCash then
        ...
        if bank then
            TriggerClientEvent('msfw-banking:openBankScreen', src)
            TriggerClientEvent('msfw-banking:successAlert', src, Lang:t('success.cash_deposit', {value = amount}))
...
RegisterNetEvent('msfw-banking:toggleCard', function(_)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('msfw-banking:doQuickWithdraw', function(amount, _)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentCash then
        ...
        if cash then
            TriggerClientEvent('msfw-banking:openBankScreen', src)
            TriggerClientEvent('msfw-banking:successAlert', src, Lang:t('success.cash_withdrawal', {value = amount}))
...
RegisterNetEvent('msfw-banking:updatePin', function(pin)
    if pin ~= nil then
        local src = source
        local xPlayer = QBCore.Functions.GetPlayer(src)
        while xPlayer == nil do Wait(0) end
        ...
        TriggerClientEvent('msfw-banking:openBankScreen', src)
        TriggerClientEvent('msfw-banking:successAlert', src, Lang:t('success.updated_pin'))
    end
end)

RegisterNetEvent('msfw-banking:savingsDeposit', function(amount)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentBank then
        ...
        TriggerClientEvent('msfw-banking:openBankScreen', src)
        TriggerClientEvent('msfw-banking:successAlert', src, Lang:t('success.savings_deposit', {value = tostring(amount)}))
...
RegisterNetEvent('msfw-banking:savingsWithdraw', function(amount)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentSavings then
        ...
        while savings == nil do Wait(0) end
        TriggerClientEvent('msfw-banking:openBankScreen', src)
        TriggerClientEvent('msfw-banking:successAlert', src, Lang:t('success.savings_withdrawal', {value = tostring(amount)}))
...
RegisterNetEvent('msfw-banking:createSavingsAccount', function()
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    local success = createSavingsAccount(xPlayer.PlayerData.citizenid)
    repeat Wait(0) until success ~= nil
    TriggerClientEvent('msfw-banking:openBankScreen', src)
    TriggerClientEvent('msfw-banking:successAlert', src, Lang:t('success.opened_savings'))
